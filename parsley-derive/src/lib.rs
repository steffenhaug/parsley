...
use syn::{DeriveInput, Data, DataEnum, Variant, Ident, Fields};
use quote::quote;
use std::mem;

fn extract_variant_info(v: &mut Variant) -> Option<(Literal, Ident, Fields)> {
    let Variant { attrs, ident, fields, .. } = v;
...
pub fn alphabet_derive(input: TokenStream) -> TokenStream {
    ...
    let match_arms_from_str: proc_macro2::TokenStream = variants.iter()
        .enumerate()
        .map(|(n, (l, _, _))| {
    ...
    let match_arms: proc_macro2::TokenStream = variants.iter()
        .enumerate()
        .map(|(n, (_, id, fs))| {
            match fs {
